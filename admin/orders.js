...
export async function newOrder(
  ...
  const [pizzas] = await promisePool.execute(
...
const result = await promisePool.execute(
  "insert into orders(cust_id, delivery_id, status, adresse) values(?,?,?,?)",
  [cust_id, delivery_id, status, adresse]
);
console.log(`Result : `,result);
let idInsertion = result[0].insertId;
console.log(`idInsertion`, idInsertion);
/*
  const [rows] = await promisePool.execute(
    "insert into orders(cust_id, delivery_id, status, adresse) values(?,?,?,?)",
    [cust_id, delivery_id, status, adresse]
 ...
 */
    const [rowsOrderPizza] = await promisePool.execute(
    "insert into order_pizza(ord_id, pizz_id, pizz_qt) values(?,?,?)", [idInsertion, pizz_id, pizz_qt]);
...
/* 
   ...
   */
  //console.log(rowsa);
  return result/* return */ /* pizzas, */ /* rows  *//*, rowsb */ /*, rowsc */;
}
